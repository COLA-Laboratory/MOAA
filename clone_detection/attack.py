...
def main():
    ...
    url_to_code = {}
    with open("/".join(args.data_file.split("/")[:-1]) + "/data.jsonl") as f:
        for line in f:
            line = line.strip()
            js = json.loads(line)
    ...
    with open(args.data_file) as f:
        for line in f:
            ...
            examples.append(
                Example(
                    idx=idx,
                    source1=url_to_code[url1],
                    source2=url_to_code[url2],
                    target=target,
                    url1=url1,
                    url2=url2,
                )
    ...
    for e in examples:
        is_success = attacker.attack(e.idx, e.source1, e.source2, e.target)
