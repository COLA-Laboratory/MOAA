...
from models import CodeT5Model
import logging
import argparse
import math
import numpy as np
            ...
            for step, batch in enumerate(bar):
                ...
                if (step + 1) % save_steps == 0 and args.do_eval:
                    ...
                    if eval_f1 > best_f1:
                        ...
                        not_f1_inc_cnt += 1
                        logger.info(
                            "F1 does not increase for %d epochs", not_f1_inc_cnt
                        )
                        if not_f1_inc_cnt > args.patience:
                            logger.info(
                                "Early stop as f1 do not increase for %d times",
                                not_f1_inc_cnt,
                            )
                            fa.write(
                                "[%d] Early stop as not_f1_inc_cnt=%d\n"
