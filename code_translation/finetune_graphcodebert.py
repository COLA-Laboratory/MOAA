...
class Example(object):
    ...
    def __init__(self, source, target, lang):
        self.source = source
        self.target = target
        self.lang = lang
...
def read_examples(filename):
    ...
    with open(source, encoding="utf-8") as f1, open(target, encoding="utf-8") as f2:
        for line1, line2 in zip(f1, f2):
            line1 = line1.strip()
            line2 = line2.strip()
            examples.append(Example(source=line1, target=line2, lang=lang))
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        attn_mask = np.zeros(
            (self.args.max_source_length, self.args.max_source_length), dtype=np.bool
        )
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval and epoch in [
                int(args.num_train_epochs * (i + 1) // 20) for i in range(20)
            ]:
                ...
                dev_bleu = round(
                    _bleu(
                        ...
                        os.path.join(args.output_dir, "dev.output"),
                    ),
                    2,
                )
                ...
                logger.info(
                    "  %s = %s " % ("xMatch", str(round(np.mean(accs) * 100, 4)))
                )
                logger.info("  " + "*" * 20)
        ...
        for idx, file in enumerate(files):
            ...
            dev_bleu = round(
                _bleu(
                    ...
                    os.path.join(
                        args.output_dir, "test_{}.output".format(str(idx))
                    ).format(file),
                ),
                2,
            )
