...
class Individual:
    def __init__(self, tokens, identifiers, pos_dict, target, model_type):
        self.tokens_ = tokens[:]
    ...
    def function_eval(self, model, tokenizer, t5_emb_model, t5_tokenizer, lang):
        for i in range(len(self.identifiers)):
            for j in self.pos_dict[i]:
                self.tokens_[j] = self.identifiers[i]
        with torch.no_grad():
            if self.model_type == "codebert" or self.model_type == "graphcodebert":
                preds = model.predict(" ".join(self.tokens_))
                top_preds = [pred[0].cpu().numpy() for pred in preds]
            elif self.model_type == "codet5":
                source_str = " ".join(self.tokens_).replace("</s>", "<unk>")
                ...
                top_preds = [pred[0].cpu().numpy() for pred in preds]
        output_fn = tokenizer.decode(
            top_preds, skip_special_tokens=True, clean_up_tokenization_spaces=False
        )
        self.obj_[0] = calc_code_bleu.get_code_bleu(self.orig_target, output_fn, lang)
        self.target = output_fn
